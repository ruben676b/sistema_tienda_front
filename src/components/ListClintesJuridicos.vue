          ...
          <form @submit.prevent="editarCliente">
            <div class="form-group">
              ...
              <input
                ...
                class="form-control"
                id="editRUC"
                v-model="clienteEditado.RUCCliente"
                required
              />
            </div>
            <div class="form-group">
              ...
              <input
                ...
                v-model="clienteEditado.NombreEmpresa"
                required
              />
            </div>
...
export default {
  data() {
    return {
      ...
      clienteEditado: {
        // Objeto para editar cliente jurídico
        RUCCliente: "",
        NombreEmpresa: "",
      },
      loading: false,
    ...
    async fetchClientesJuridicos() {
      ...
      try {
        const response = await axios.get(
          "http://localhost:3000/api/v1/clientes-juridicos"
        );
    ...
    async borrarCliente(idCliente) {
      try {
        const response = await axios.delete(
          `http://localhost:3000/api/v1/clientes-juridicos/${idCliente}`
        );
        if (response.data.success) {
          this.clientesJuridicos = this.clientesJuridicos.filter(
            (cli) => cli.IdCliente !== idCliente
          );
        } else {
    ...
    async editarCliente() {
      try {
        const response = await axios.patch(
          `http://localhost:3000/api/v1/clientes-juridicos/${this.clienteEditado.IdCliente}`,
          this.clienteEditado
        );
        if (response.data.success) {
