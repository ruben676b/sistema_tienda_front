         ...
         <div class="container-fluid">
            <div class="row">
               ...
               <ListadoProductos :productosFiltrados="productosFiltrados"
                  :isProductoSeleccionado="isProductoSeleccionado" :agregarProducto="agregarProducto"
            ...
            <div class="row">
               ...
               <div class="col-lg-6">
                  <div v-if="tipoCliente === 'juridico'">
                     <ClienteJuridico @cliente-seleccionado="actualizarIdCliente" v-if="tipoCliente === 'juridico'" />
                  </div>
                  <div v-if="tipoCliente === 'natural'">
                     <ClienteNatural @cliente-seleccionado="actualizarIdCliente" v-if="tipoCliente === 'natural'" />
                  </div>
               </div>
            </div>
            <br>
   ...
   methods: {
      ...
      async registrarVenta() {
         ...
         if (!this.idCliente && this.tipoComprobante !== 'ticket') {
            Swal.fire('Error', 'Por favor seleccione un cliente o cambie a tipo de comprobante ticket', 'error');
            return;
         }

         try {
            const response = await axios.post('http://localhost:3000/api/v1/ventas/registrar', datosVenta);
            if (response.data.success) {
               Swal.fire('Éxito', 'Venta registrada correctamente', 'success');
               this.limpiarFormulario(); // Ahora esto debería funcionar
            } else {
               Swal.fire('Error', response.data.message || 'Error al registrar la venta', 'error');
            }
         } catch (error) {
