<template>
  <div>
    ...
    <div class="mt-3">
      ...
      <button
        v-if="clienteSeleccionado && !modoEdicion"
      ...
      if (this.cliente.NombreCliente.trim().length >= 1) {
        try {
          ...
          this.sugerencias = clientes
            .filter((cliente) =>
              cliente.NombreCliente.toLowerCase().includes(
                this.cliente.NombreCliente.trim().toLowerCase()
              )
            )
            .slice(0, 5);
        } catch (error) {
